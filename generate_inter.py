...
vq_parser.add_argument("-ips",  "--iterations_per_second", type=int, help="Number of iterations per second", default=100, dest='iterations_per_second')
...
args.default_image_size = 512  # >8GB VRAM
if not torch.cuda.is_available():
    args.default_image_size = 256  # no GPU found
elif get_device_properties(0).total_memory <= 2 ** 33:  # 2 ** 33 = 8,589,934,592 bytes = 8 GB
    args.default_image_size = 318  # <8GB VRAM
...
if args.make_video or args.make_zoom_video:
    if not os.path.exists(f'steps{args.gpu_id}'):
        os.mkdir(f'steps{args.gpu_id}')
...
if args.video_style_dir:
    ...
    cwd = os.getcwd()
    args.output = os.path.join(cwd, f'steps{args.gpu_id}', filename)
...
def ascend_txt():
    ...
    if args.make_video:    
        ...
        imageio.imwrite(f'./steps{args.gpu_id}/' + str(i) + '.png', np.array(img))
...
if args.lyrics:
    ...
    for i in range(len(lyrics_cp)+1):
        if i==0:
            a = 0
            b = lyrics_cp[i]
        elif i==len(lyrics_cp):
            a = lyrics_cp[i-1]
            b = total_seconds
        else:
            a = lyrics_cp[i-1]
        ...
        seg_length = b - a
        print(f'segment length :{seg_length}, a:{a}, b:{b}')
        audio_seg = audio[int(a * (sr)):int(b * (sr))] 
        print(f'length of audio segment: {len(audio_seg)}')
        audio_lst.append(audio_seg)
    
        if i==0:
            audio_length.append(seg_length)
            iterations_num.append(seg_length * args.iterations_per_second)
        else:
            audio_length.append(seg_length/2)
            audio_length.append(seg_length/2)
...
for a in range(len(audio_length)):
            ...
            while True:            
                ...
                if args.make_zoom_video:
                    if i % args.zoom_frequency == 0:
                        ...
                        img = np.array(out.mul(255).clamp(0, 255)[0].cpu().detach().numpy().astype(np.uint8))[:,:,:]
                        img = np.transpose(img, (1, 2, 0))
                        imageio.imwrite(f'./steps{args.gpu_id}/' + str(j) + '.png', np.array(img))
    ...
    if args.make_video or args.make_zoom_video:
        ...
        for i in range(init_frame,last_frame):
            temp = Image.open(f"./steps{args.gpu_id}/"+ str(i) +'.png')
            keep = temp.copy()
            frames.append(keep)
            temp.close()
